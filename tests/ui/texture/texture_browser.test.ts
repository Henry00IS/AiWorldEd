...
} from '../../../src/ui/texture/texture_browser.js';
import {
  TEXTURE_BROWSER_THUMB_CLASS,
  TEXTURE_BROWSER_GRID_CLASS,
  TEXTURE_BROWSER_MIN_TRACK_PX,
} from '../../../src/ui/texture/texture_browser_styles.js';
import { FloatingPanelStack } from '../../../src/ui/floating_panel_stack.js';
import { TextureBrowserEntry } from '../../../src/texture/library/texture_browser_entry.js';
...
describe('TextureBrowser', () => {
  ...
  it('should invoke open-folder when the toolbar button is clicked', () => {
    browser.show();
    const openButton = host.querySelector('button[aria-label="Open Folder"]') as HTMLButtonElement;
    openButton.click();
  ...
  it('should render thumbs as background-image squares with names below', () => {
    ...
    expect((thumbs[0] as HTMLElement).style.backgroundImage).toContain('blob:brick');
    expect((thumbs[1] as HTMLElement).style.backgroundImage).toContain('blob:floor');
    expect((thumbs[0] as HTMLElement).classList.contains(TEXTURE_BROWSER_THUMB_CLASS)).toBe(true);
  });

  it('should report selection when a tile is clicked', () => {
    browser.setEntries([createEntry('metal.png', 'metal', 'blob:metal')], null, 'Pack');
    browser.show();
    const tile = host.querySelector('[aria-label="metal.png"]') as HTMLElement;
    tile.click();
  ...
  it('should expose a resize handle and enforce minimum panel size', () => {
    browser.show();
    const panel = browser.getRootElement();
    const handle = host.querySelector('[aria-label="Resize browser"]') as HTMLElement;
    ...
    expect(panel.style.width).toBe(`${TEXTURE_BROWSER_DEFAULT_WIDTH_PX + 80}px`);
    expect(panel.style.height).toBe(`${TEXTURE_BROWSER_DEFAULT_HEIGHT_PX + 40}px`);
  });
