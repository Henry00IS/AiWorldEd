...
export class ControllerCameraFit {
  ...
  private isEditorOverlayRootName(name: string): boolean {
    if (name === 'gizmo_transform' || name === 'gizmo_transform_viewport') return true;
    if (name === 'gizmo_bounds' || name === 'grids_root') return true;
    if (name === 'infinite_grid_2d' || name === 'infinite_grid_3d') return true;
    return name.startsWith('gizmo_transform');
  }
