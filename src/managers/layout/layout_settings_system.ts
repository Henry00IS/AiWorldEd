...
export function createLayoutSettingsSystem(deps: LayoutSettingsCreateDeps): LayoutSettingsSystemParts {
  ...
  deps.viewport3D.setOrbitSelectionInvertYAxis(settingsStore.getMouseSettings().orbitSelectionInvertYAxis);
  ...
  const settingsUnsubscribe = settingsStore.subscribe((snapshot) => {
    ...
    deps.viewport3D.setOrbitSelectionInvertYAxis(snapshot.mouse.orbitSelectionInvertYAxis);
